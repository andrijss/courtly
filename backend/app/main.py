...
def _ensure_schema() -> None:
    with engine.begin() as connection:
        ...
        if "image_url" not in columns:
            connection.execute(text("ALTER TABLE courts ADD COLUMN image_url VARCHAR(600)"))
        review_columns = {row[1] for row in connection.execute(text("PRAGMA table_info(reviews)"))}
        if "booking_id" not in review_columns:
            connection.execute(text("ALTER TABLE reviews ADD COLUMN booking_id VARCHAR(36)"))
        existing_indexes = {row[1] for row in connection.execute(text("PRAGMA index_list(reviews)"))}
        if "uq_review_user_booking" not in existing_indexes:
            connection.execute(text("CREATE UNIQUE INDEX uq_review_user_booking ON reviews(user_id, booking_id)"))
        policy_columns = {row[1] for row in connection.execute(text("PRAGMA table_info(policies)"))}
        if "role" not in policy_columns:
            connection.execute(text("ALTER TABLE policies ADD COLUMN role VARCHAR(60) DEFAULT 'admin' NOT NULL"))
...
def _seed_rbac_policies(db: Session) -> None:
    ...
    default_policies = [
            ...
            ("moderator", "/courts/:courtId/availability", "read", "allow", "ownership:all"),
            ...
            ("user", "/me/profile/request-data-deletion", "create", "allow", "ownership:self"),
    ...
    for role, resource, action, effect, condition in default_policies:
        existing_policy = db.scalar(
            select(Policy).where(Policy.role == role, Policy.resource == resource, Policy.action == action)
        )
        if existing_policy is None:
            db.add(Policy(role=role, resource=resource, action=action, effect=effect, condition=condition))
