...
class Booking(Base):
    ...
    hold_token: Mapped[str | None] = mapped_column(String(80), unique=True, nullable=True, index=True)
...
class Review(Base):
    ...
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id"), index=True)
    court_id: Mapped[str] = mapped_column(ForeignKey("courts.id"), index=True)
    booking_id: Mapped[str | None] = mapped_column(ForeignKey("bookings.id"), index=True, nullable=True)
